...
def main():
    ...
    profiles_all = generate_random_pulse_profiles(
        num_profiles=1060, t_span=t_span, alpha_max=0.1, seed=GLOBAL_SEED,
        min_width=0.2, max_width=5.0
    )
    rng = np.random.default_rng(GLOBAL_SEED)
    perm = rng.permutation(len(profiles_all))
    idx_train = perm[:1000]
    idx_val   = perm[1000:1030]     # 30 本
    ...
    # SIREN（論文準拠）
    omega_siren = 20.0
    net_S = SIREN_SineNN_ZIG(hidden_dim=4, omega_0=omega_siren, num_hidden_layers=2)
